...
class VideoRender:IRender {
    ...
    /**
     * 将顶点坐标和纹理坐标，转换成FloatBuffer，用以OpenGL ES程序使用
     */
    private fun initPos(){
        val bb = ByteBuffer.allocateDirect(mVertexCoors.size * 4)
        bb.order(ByteOrder.nativeOrder())
        //将坐标数据转换成FloatBuffer，用来传入给OpenGL ES程序
    ...
    private fun initDefMatrix(){
        ...
        if(mVideoWidth != -1 && mVideoHeight!= -1
            ...
            mMatrix = FloatArray(4*4)
            val prjMatrix = FloatArray(4*4)
            ...
            if(mWorldWidth > mWorldHeight){
                ...
                if(originRatio > worldRatio){
                    mHeightRatio = originRatio/worldRatio
                    Matrix.orthoM(prjMatrix, 0,-mWidthRatio,mWidthRatio,-mHeightRatio,mHeightRatio,3f,5f)
    ...
    private fun  createGlProgram(){
        if(mProgram == -1){
            ...
            //创建OpenGL ES程序，注意：需要在opengl线程中创建，否则无效
    ...
    private fun doDraw(){
        ...
        GLES20.glUniformMatrix4fv(mVertexMatrixHandler,
            1,false,mMatrix,0)
